...
use crate::mcore::{config::Config, errors::*, item::Item};
...
fn get_item(script_dir: &Path) -> Result<Item> {
    ...
    let mut item: ScriptItem =
        toml::from_str(&itemdata).map_err(|e| Error::with_chain(e, "Failed parsing item.toml"))?;
...
pub fn get(config: &Config) -> Vec<Item> {
    ...
    for dir in config
        .get::<Vec<String>>(&["core", "extra_plugin_directories"])
        .unwrap()
        .iter()
    {
        ...
        for entry in entries {
            if let Ok(entry) = entry {
                ...
                match get_item(&entry_path) {
                    ...
                    Err(error) => {
                        warn!(
                            "Unable to load custom script at {:?}: {}",
                            entry_path,
                            error.display_chain()
                        );
                    }
