...
impl Action for LinuxDesktopEntry {
    ...
    fn runnable_arg(&self) -> bool {
        self.exec
            .iter()
            .find(|arg| (*arg == "%f" || *arg == "%F" || *arg == "%u" || *arg == "%U"))
            .is_some()
    }
...
impl LinuxDesktopEntry {
    ...
    fn get(filepath: &Path) -> Result<LinuxDesktopEntry> {
        let config = Ini::load_from_file_opt(
            filepath,
            ini::ParseOption {
                enabled_quote: false,
        ...
        Ok(LinuxDesktopEntry {
            name: config
                ...
                .ok_or(Error::from("No name key found in desktop entry"))?
                .into(),
            comment: Some(
                config
                    .get_from_or(Some("Desktop Entry"), "Comment", "")
                    .into(),
            ),
            exec: shlex::split(exec_str).ok_or(Error::from("Unable to parse Exec key"))?,
    ...
    fn get_all(config: &Config) -> Vec<LinuxDesktopEntry> {
        let directories = config
            .get::<Vec<String>>(&["linux_desktop_entry", "directories"])
            .unwrap();
            ...
            for entry in entries {
                if let Ok(entry) = entry {
                    ...
                    match LinuxDesktopEntry::get(&entry_path) {
                        ...
                        Err(error) => {
                            warn!(
                                "Unable to load desktop entry at {:?}: {}",
                                entry_path,
                                error.display_chain()
                            );
                        }
...
pub fn get(config: &Config) -> Vec<Item> {
    LinuxDesktopEntry::get_all(config)
        .into_iter()
        .map(|action| {
